...
author_patterns = patterns('',
   ...
   url(r'^(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)/$', EntryAuthorDayArchiveView.as_view(), name='blogs_entry_author_archive_day'),
   url(r'^(?P<year>\d+)/(?P<month>\d+)/$', EntryAuthorMonthArchiveView.as_view(), name='blogs_entry_author_archive-month'),
   url(r'^(?P<year>\d+)/$', EntryAuthorYearArchiveView.as_view(), name='blogs_entry_author_archive_year'),
   url(r'^today/$', EntryAuthorTodayArchiveView.as_view(), name='blogs_entry_author_archive_today'),
   url(r'^$', EntryAuthorListView.as_view(), name='blogs_author_entry_list'),
)

urlpatterns = patterns('',
    url(r'^(?P<year>\d+)/(?P<month>\d+)/(?P<day>\d+)/$', EntryDayArchiveView.as_view(), name='blogs_entry_archive_day'),
    url(r'^(?P<year>\d+)/(?P<month>\d+)/$', EntryMonthArchiveView.as_view(), name='blogs_entry_archive-month'),
    url(r'^(?P<year>\d+)/$', EntryYearArchiveView.as_view(), name='blogs_entry_archive_year'),
    url(r'^$', EntryListView.as_view(), name='blogs_entry_list'),
    url(r'^today/$', EntryTodayArchiveView.as_view(), name='blogs_entry_archive_today'),
